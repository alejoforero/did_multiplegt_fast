...
capture program drop did_multiplegt_check2
program did_multiplegt_check2, eclass
	version 12.0
...
sa `saving'
preserve
loc filenumber=`=clock("`c(current_time)'", "hms")'
*di as red `maxprocessors'
*cap log close
*log using parallel_map_log, replace
*set tracedepth 2
*set trace on
cap did_multiplegt_core2
cap did_multiplegt_estim2

noi: parallel_map, val(1/`maxprocessors') id(`filenumber') verbose max(`maxprocessors') cores(4) force: did_multiplegt_parallel , varlist(`varlist') threshold_stable_treatment(`threshold_stable_treatment') trends_nonparam(`trends_nonparam') trends_lin(`trends_lin') controls(`controls') counter(`counter') placebo(`placebo') dynamic(`dynamic') breps(`breps') cluster(`cluster')  switchers(`switchers') `longdiff_placebo' `count_switchers_tot' `count_switchers_contr' `robust_dynamic' discount(`discount') filename(`saving') seed(`seed') paralleloffset(`paralleloffset') maxprocessors(`maxprocessors')
*set trace off
*log close
clear

forvalues i=1/`breps'{
	matload didmgt_bootstrap_`i', p("`c(tmpdir)'") over
	cap n erase "`c(tmpdir)'\didmgt_bootstrap_`i'.dta"
}
